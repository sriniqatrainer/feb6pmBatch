...
public class OHRM_HomePage_LinksCount_DisplayingTheLinkNames {
...
System.out.println(" The actual title of the OrangeHRM WebPage is :"+actual_OrangeHRMWebPageTitle);
		...
		// identifying the links on the Home Page
		List<WebElement>homePageLinks=driver.findElements(By.tagName("a"));
		
		int homePageLinks_Count=homePageLinks.size();
		System.out.println(" The total number of links on the HomePage are : "+homePageLinks_Count);
		
		
		for(int k=0;k<homePageLinks_Count;k++)
		{
			if(homePageLinks.get(k).isDisplayed())
			{
			
			String linkName=homePageLinks.get(k).getText();
		...
		Thread.sleep(10000); // Explicit Wait - WebDriver will wait for the completion of the time
